...
def data_generator(data_path, domain_id, dataset_configs, hparams):
    ...
    len_train_lodaer = len(train_dataset)
    train_dataset,val_dataset = torch.utils.data.random_split(train_dataset, [int(0.70*len(train_dataset)),
                                                                              len(train_dataset) - int(0.70*len(train_dataset))])
...
def custom_data_generator(data_path, domain_id, dataset_configs, batch_size = 64):
    # loading path
    train_dataset = torch.load(os.path.join(data_path, "train_" + domain_id + ".pt"))
    test_dataset = torch.load(os.path.join(data_path, "test_" + domain_id + ".pt"))
